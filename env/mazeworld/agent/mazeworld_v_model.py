...
class MazeworldVModel(IVModel[State, Reward, Option[OptionData]]):
    ...
    def forward(self, state: State, option: Option) -> torch.Tensor:
        ...
        point: Point = option.value 
        grid: np.ndarray = self.env._point_to_grid(point) 
        # np.ndarray[float] : [ydim, xdim, 1]
        representation: np.ndarray = np.concatenate((state, grid), axis=-1)
        ...
        # torch.Tensor[float, self.device] : [ydim, xdim, 4]
        representation: torch.Tensor = representation.unsqueeze(0).permute((0, 3, 1, 2))
        # torch.Tensor[float, self.device] : [1, 4, ydim, xdim, 4]
    ...
    def optimize(self, inputs: List[Tuple[State, Option]], targets: List[Reward]) -> None:
        ...
        for (state, option) in inputs:
            preds.append(self.forward(state, option))
