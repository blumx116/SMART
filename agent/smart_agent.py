...
GoalManager = TypeVar("GoalManager")
GoalBasedAgent = TypeVar("GoalBasedAgent")


class SMARTAgent:
    def __init__(self, goal_manager: GoalManager, low_level_agent: GoalBasedAgent):
    ...
    def reset(self, env: Environment, goal: Goal) -> None:
        ...
        self.__terminal_goal = Node(goal)
    ...
    def _announce_abandon_goal(self, goal: Node[Goal]) -> None:
        self.goal_manager._observe_abandon_goal(goal)

    def _announce_set_current_goal(self, goal: Node[Goal]) -> None:
        self.goal_manager._observe_set_current_goal(goal)
