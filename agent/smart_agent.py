...
from agent.memory.trees import BinaryTreeNode
...
class SmartAgent:
    def __init__(self, generator, planning_terminator, selector, simple_executor):
        self.generator = generator
        self.planning_terminator = planning_terminator
        self.selector = selector
        self.simple_executor = simple_executor

    def reset(self, env: Environment, goal: RawGoal):
        """
            ...
            goal: RawGoal
                ...
                external sources
        """
        self.generator.reset(env)
        self.planning_terminator.reset(env)
        self.selector.reset(env)
        self.simple_executor.reset(env)
    ...
    def get_actionable_goal(self, state: State) -> Goal:
        ...
        if self.current_actionable_goal is None:
            if self.should_abandon(state, self.current_actionable_goal.value):
                self.abandon_subgoal(self.current_actionable_goal)
                return self.get_actionable_goal(state)
            else: return self.current_actionable_goal.value
    ...
    def plan(self, state: State) -> Goal:
        ...
        if not self.should_plan_for(state, next_goal.value):
            self.current_actionable_goal = next_goal
            return self.current_actionable_goal.value
        else:
            chosen_subgoal: Goal = self.get_subgoal(state, next_goal.value)
    ...
    def abandon_subgoal(self, goal_node: BinaryTreeNode[Goal]) -> None:
        ...
        assert goal_node.value != self.highest_goal
